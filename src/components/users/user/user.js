...
import './user.scss'
const User = ({ user, onClickInvite, isInvitesId, getUserById, path }) => {
    return (
        <li>
            <div>
                <img className="avatar" src={user.avatar} alt="User" />
                <div>
                    <Link to={`${path}${user.id}`} id='fullName' onClick={() => { getUserById(user.id) }}>{`${user.first_name} ${user.last_name}`}</Link>
                    <p>
            ...
            {
                isInvitesId
                    ? <Invited userId={user.id} onClickInvite={onClickInvite}>
                        <p id='inviteCondition'>-</p>
                    </Invited>
                    : <Invited userId={user.id} onClickInvite={onClickInvite}>
                        <p id='inviteCondition'>+</p>
                    </Invited>
